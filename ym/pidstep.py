...
# PIDNet 구조 불러오기 (경로에 맞게 import)
from model.PIDNet.models.pidnet import get_seg_model

class Config:
    class MODEL:
        NAME = 'pidnet-l'  # 또는 'pidnet-m', 'pidnet-s'
        PRETRAINED = 'PIDNet_L_Cityscapes_test.pt'
    class DATASET:
        NUM_CLASSES = 19
...
class SegmentationApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("보행가능구역-pidnet")
        self.capture = None
        self.timer = QTimer()
    ...
    def load_model(self):
        ...
        #checkpoint = torch.load("PIDNet_L_Cityscapes_test.pt", map_location='cpu')
        #model.load_state_dict(checkpoint)
        model.to(self.device)
        model.eval()
        return model
    
    def decode_segmap(self, seg_map):
        color_map = self.color_map
        h, w = seg_map.shape
        seg_color = np.zeros((h, w, 3), dtype=np.uint8)
        for label in range(len(color_map)):
            seg_color[seg_map == label] = color_map[label]
        return seg_color

    ...
    def update(self):
        while self.running:
            ...
            with torch.no_grad():
                ...
                dilated_road_mask = self.apply_dynamic_dilation(edges, depth_mask, 501, 31)
                gap_mask = self.apply_dynamic_dilation(edges, depth_mask, 51, 7)
                dilated_road_mask = dilated_road_mask - gap_mask
                dilated_road_mask = cv2.bitwise_and(dilated_road_mask, mask)
                road_mask = mask - dilated_road_mask
                result_mask = np.full_like(mask, 0)
                result_mask[road_mask == 255] = 150
                result_mask[dilated_road_mask == 255] = 255

            # 시각화
            seg_rgb = cv2.cvtColor(result_mask, cv2.COLOR_GRAY2RGB)
